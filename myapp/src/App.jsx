    ...
    <div className="min-h-screen bg-black text-white overflow-x-hidden font-mono relative">
      ...
      {clickCombo > 1 && (
        <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 pointer-events-none z-50">
          <div className={`pixel-font text-4xl md:text-6xl combo-text animate-pulse ${clickCombo === 20 ? 'text-orange-500 glow' : 'text-yellow-400'
            }`}>
          ...
          <div className="flex justify-center gap-3 mt-8">
            {testimonials.map((_, i) => (
              <button
                ...
                className={`w-4 h-4 border-2 border-blue-500 transition-all ${i === currentTestimonial ? 'bg-blue-500' : 'bg-transparent'
                  }`}
              />
            ))}
          </div>
        ...
        <div className="max-w-3xl w-full">
          ...
          <div className="bg-black border-4 border-purple-500 pixel-border p-8">
            {highScores.map((entry, i) => (
              <div
                key={i}
                className={`flex justify-between items-center py-4 border-b-2 ${i === highScores.length - 1 ? 'border-transparent' : 'border-purple-500/30'
